...
if [ $BINARIZED -eq 1 ]; then
    ...
    --tokenizer_type auto \
    --tokenizer_name nyu-mll/roberta-base-1B-1 \
    --cache_dir $CACHE_DIR \
...
python -m torch.distributed.launch \
	...
	--n_epoch $EPOCH \
        ...
        --freeze_pos_embs \
        --dump_path /logs/distilbert/$FILENAME.init-rb-1B_label-rb-original_ce-loss-1_roberta \
        --data_file $BASE_DIR/binarized_$FILENAME.roberta-base-1B-1.pickle \
        --token_counts $BASE_DIR/token_counts.roberta-base-1B-1.pickle \
        --checkpoint_epoch_interval 1 \
        --student_pretrained_weights /logs/weight_initialization/rb_0247911.pth \
